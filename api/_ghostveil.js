...
async function fetchTokenHolderIntel(mint) {
  ...
  if (topTokenAccounts.length) {
    const ownerPayload = await solanaRpc("getMultipleAccounts", [topTokenAccounts, { encoding: "jsonParsed" }]);
    owners = (ownerPayload?.value || []).map((account) => account?.data?.parsed?.info?.owner).filter(Boolean);
  }
  ...
  return {
    ...
    evidence: [
      ...
      owners.length ? `Largest sampled owner wallets: ${[...uniqueOwners].slice(0, 4).map(shortAddress).join(", ")}.` : "No owner-wallet sample available.",
    ],
    directEvidence: [
...
async function fetchWalletActivityIntel(wallet) {
  ...
  const repeated = [...counterparties.entries()].filter(([, count]) => count >= 2).sort((a, b) => b[1] - a[1]).slice(0, 4);
  ...
  return {
    ...
    evidence: [
      ...
      sigs.length ? `Recent activity sample: ${sigs.length} signature(s), newest ${newestSample || "unknown"}, oldest in sample ${oldestSample || "unknown"}.` : "No recent signatures returned by public RPC for this wallet.",
      repeated.length ? `Repeated counterparties in recent transaction sample: ${repeated.map(([address, count]) => `${shortAddress(address)} (${count}x)`).join(", ")}.` : "No repeated counterparties found in the recent transaction sample.",
    ],
    directEvidence: [
